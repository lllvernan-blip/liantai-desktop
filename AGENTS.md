...
- 差量的两个前提：缓存 `%LOCALAPPDATA%\liantai-desktop-updater\installer.exe`（上一版安装包）在，且**源上旧版的 `.blockmap` 不删**。generic 源支持 `multipart/byteranges` 才是真差量（不支持就优雅退化为全量，不报错）；GitHub 资产 CDN 对多段 Range 返回 501，但 `BaseGitHubProvider` 写死单段逐段请求（206 可用），所以 GitHub 源差量可用。
  - **差量省不了多少，别拿它当卖点**：2026-09-24 实测 0.0.12→0.0.13，全量 84.6MB、待下 56.5MB（2756 个变化块），只省约三分之一。
  - 差量失败会留痕，且会自己退全量：`logs/startup.log` 里 `update-error :: Cannot download differentially, fallback to full download: Error: sha512 checksum mismatch, expected <远端清单的>, got <本地组装的>`——这是差量包组装完后校验没过（不是源坏了、不影响可用性），当晚那次整整跑了 21 分钟 / 前后约 141MB。**判定「差量基准不对」还是「CDN 分段给错字节」只能看下一对版本**：再失败一次才值得去摸 `differentialDownloader`。
- 网络现实：国内直连 GitHub 时 `checkForUpdates` 可能直接 `ERR_CONNECTION_TIMED_OUT`（真机见过）。这不是 bug：失败会按规则重试且不阻断使用；要稳定就换源（`LIANTAI_UPDATE_FEED`）。
- **备用源与校验下载分离**：主源永远是包内 `app-update.yml`（GitHub 官方）；它连不通时 `update.js` 按 `BACKUP_FEEDS` 顺序换源，全试过才落 error（下一轮从主源重新开始）。列表在 `update.js` 顶部，2026-09-25 夜本机重测：`ghproxy.net` / `gh-proxy.com` / `gh.xxooo.cf` / `gh.nxnow.top` 可取到 `latest.yml` 且支持单段 Range（206）；`gh.ddlc.top` 连续 429 已换掉，`gh-proxy.net` 能取元数据但 Range 给全量（200）不能走差量，直连 github.com 仍是连接超时。**换线路只认两个硬指标：元数据能取到、支持单段 Range**（不满足就只能全量下载）。**校验与下载必须分开（2026-09-21 阿楠拍板）**：元数据（带 sha512）取自 `BACKUP_FEEDS[i]`，安装包地址改写指向 `BACKUP_FEEDS[i+1]`（`installCrossSourceHook` 覆写 `getUpdateInfoAndProvider`，只取原地址末段拼到我们选的下载源上，元数据里写绝对地址也拨回来）。同一个源改不了两边，sha512 校验才不是自证；要作恶得两个源同时动手。由此得一条排列表的规矩：**相邻两条同时活着才算一条能走通的路**，只测「某条单源可用」没意义。设了 `LIANTAI_UPDATE_FEED` 就只认那一个源，既不兜底也不拆。
